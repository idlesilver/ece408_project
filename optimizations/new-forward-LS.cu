...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    int W_num = ceil(W_out / (TILE_WIDTH1 * 1.0));
    int H_num = ceil(H_out / (TILE_WIDTH1 * 1.0));
    
    int b = blockIdx.x, m = blockIdx.y;
    int w = (blockIdx.z % W_num) * TILE_WIDTH1 + threadIdx.x;
    int h = (blockIdx.z / W_num) * TILE_WIDTH1 + threadIdx.y;
...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    int W_num = ceil(W_out / (TILE_WIDTH2 * 1.0));
    int H_num = ceil(H_out / (TILE_WIDTH2 * 1.0));
    
    int b = blockIdx.x, m = blockIdx.y;
    int w = (blockIdx.z % W_num) * TILE_WIDTH2 + threadIdx.x;
    int h = (blockIdx.z / W_num) * TILE_WIDTH2 + threadIdx.y;
...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    if(M < 8){
        ...
        std::cout<<" using kernel 1: "<<TILE_WIDTH1 <<std::endl;
        dim3 dimGrid(B, M, ceil(W_out / (TILE_WIDTH1 * 1.0)) * ceil(H_out / (TILE_WIDTH1 * 1.0)));
        dim3 dimBlock(TILE_WIDTH1, TILE_WIDTH1, 1);

        conv_forward_kernel1<<<dimGrid, dimBlock>>>(device_y, device_x, device_k, B, M, C, H, W, K);
    }else{
        
        std::cout<<" using kernel 2: "<<TILE_WIDTH2 <<std::endl;
        dim3 dimGrid(B, M, ceil(W_out / (TILE_WIDTH2 * 1.0)) * ceil(H_out / (TILE_WIDTH2 * 1.0)));
