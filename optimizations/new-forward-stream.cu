...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    for (int i = 0; i < STREAM_NUM; ++i) {
        conv_forward_kernel<<<dimGrid, dimBlock, 0, stream[i]>>>(device_y + i*y_stream_size, device_x + i*x_stream_size, device_k, B, M, C, H, W, K);
    }
...
__host__ void GPUInterface::conv_forward_gpu_epilog(float *host_y, float *device_y, float *device_x, float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    for (int i = 0; i < STREAM_NUM; ++i) {
        cudaMemcpyAsync((void*)(host_y + i*y_stream_size), (void*)(device_y + i*y_stream_size) , y_stream_size * sizeof(float), cudaMemcpyDeviceToHost, stream[i]);
    }
