 ...
 */
 ...
 #define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
     ...
     */
     ...
     const int x_out_width = TILE_WIDTH + K - 1;
     
     // FIXME: x_out_width cannot used as const, it is const expersion, which cannot be used to declare array!
     extern __shared__ float shareX[2000];
     extern __shared__ float shareW[5000];
 
    // use one place instead
    //  extern __shared__ float sc[];

    // float* shareX = &sc[0];
    // float* shareW = &sc[x_out_width*x_out_width];
        ...
        for (p = 0; p < K; ++p) {
            for (q = 0; q < K; ++q) {
                ...
                if(((h0+p) < x_out_width) && ((w0+q) < x_out_width)){
                    res+=shareX[(h0+p)*(x_out_width) + (w0+q)] * shareW[p*K+q];
                }
            }
        }
        __syncthreads();
